  ...
  return (
    ...
    >
      ...
      <div className="relative max-w-[1280px] mx-auto px-6 lg:px-10">
        ...
        {/* Header */}
        <motion.div className="text-center mb-14" variants={fadeUp} initial="hidden" animate={inView ? "show" : "hidden"}>
          <SectionBadge variant="white">{t("badge")}</SectionBadge>
          <h2 className="font-display font-bold text-white mt-2" style={{ fontSize: "clamp(1.7rem, 3vw, 2.6rem)" }}>
            {t("title")}
          </h2>
          <p className="text-white/60 mt-3 max-w-md mx-auto text-sm">
            {locale === "fr" ? "Ce que nos familles disent de nous" : locale === "en" ? "What our families say about us" : "Dzam ba balɛ́g ba sukul"}
          </p>
        </motion.div>
          ...
          {items.map((item, i) => (
            ...
            >
              ...
              <blockquote className="flex-1 mb-6">
                <p className={`font-display italic leading-relaxed text-[0.97rem] ${i === 1 ? "text-white" : "text-white/90"}`}>
              ...
              {i === 1 && (
                <div className="absolute top-5 right-5 bg-white/20 text-white text-[10px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-full">
        ...
        {/* Bottom stat strip */}
        <motion.div
          className="mt-14 grid grid-cols-3 gap-4 max-w-lg mx-auto text-center"
          initial={{ opacity: 0, y: 16 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          transition={{ delay: 0.7, duration: 0.5 }}
        >
          {[
            { value: "100%", label: locale === "fr" ? "Satisfaction" : "Satisfaction" },
            { value: "120+", label: locale === "fr" ? "Familles" : "Families" },
            { value: "5★",   label: locale === "fr" ? "Note moyenne" : "Avg. rating" },
          ].map((s, i) => (
            <div key={i}>
